...
test("종합: 성분표에 없는 값(undefined)은 '정보 없음' + 판정 불가 — 0으로 오판하지 않음", () => {
  // 당류가 표에 없던 성분표: 당뇨는 판정 불가(unknown), 고혈압은 나트륨으로 정상 판정
  const r = judgeMeal({ sugar_g: undefined, sodium_mg: 1790, purine_level: "low" },
    { diseases: ["diabetes", "hypertension"], severity: "mild" });
  assert.equal(r.results[0].color, "unknown");
  assert.match(r.results[0].detail, /당류 정보 없음/);
  assert.equal(r.results[1].color, "red");
  assert.equal(r.overall, "red");
});
